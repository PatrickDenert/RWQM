...
void loop(void) {
  const unsigned long fifthteenmin = 15UL * 60UL * 1000UL;
  static unsigned long timekeep = 0UL;  // initialize such that a reading is due the first time through loop()
  float temperature = 0;
  float turbidity = 0;
  float tds = 0;
  ...
  timekeep = timekeep + fifthteenmin;
  }
}
